...
- [x] 3×3 모듈 전 셀 조립 및 스윕 검증 (8/30 전 코일 스윕으로 코일 0/4/5/6 극성 보정)
- [ ] 셀 간 자기 간섭(크로스토크) 대책 — 상단 철 와셔 래치 / 철판 격벽
- [x] 상위 제어기(Pi) 연동 — 카메라 ArUco 좌표 인식 → 창 패턴 출력 (8/25 리더 v1, `pi/reader.py`)
- [ ] 9핀 래칭 재현성 개선 (핀별 기계 편차, 유지력)
- [ ] 카메라 문자 인식(OCR) → 점자 변환 연동
- [ ] 당사자(시각장애 학생·특수교사) 촉각 판독성 평가
